...
|---|---|---|---|---|---|
| 2.1 | **Rescue missions.** A creature is stranded (blinking flare + its own beacon); low-slow flyby scoops it; bring it home. Reverse delivery — doubles the mission verbs with mostly existing code | FS | Search & rescue is real aviation | "Someone needs us!" | M |
| 2.2 | **Sky discoveries.** Hot-air balloons, a geese flock that scatters, a train on a loop, a whale offshore. Honk → they respond | FS | The world below is alive | Exploration finally pays | M |
...
| 2.4 | **Space station docking-lite.** A small station on the route; glide into its glowing ring *gently* (speed-gated like the home-field return) → airlock cheer, buddy waves through the window | SS | Docking = matching speeds; precision | A second destination type in space | M/L |
| 2.5 | **Constellation collection.** Star-gems collected per trip light up a constellation drawn in the sky (per profile, persistent). Finish one → it glows forever | SS | Constellations are a thing; stars are collectible knowledge | Persistent sky-building = the retention engine, aimed upward | M |
