...
| # | Item | Game | Why |
|---|---|---|---|
| 3.1 | ✅ *shipped, now default-on* — **Manual throttle.** A green power lever on the right edge: throttle back to slow/descend, push up to climb — the power lesson SS already teaches, now in FS. `CONFIG.manualThrottle` defaults **true** (Dad asked for it); `?throttle=0` hands it back to the auto-throttle. Still capped at `THRUST_MAX`, so a firewalled sustained pull *still stalls* (proven in `test/e2e-throttle.js`); a ground-roll floor means idle can never strand the plane | FS | The deepest deferred lesson |
